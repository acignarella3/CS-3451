...
class Camera {
    ...
    constructor(public pos: Vector, lookAt: Vector, hsize: number, vsize: number, distance: number) {
        var down = new Vector(0.0, -1.0, 0.0);
        this.forward = Vector.times(distance, Vector.norm(Vector.minus(lookAt, this.pos)));
...
class MovingSphere extends Sphere {
    ...
    getCenter(time: number) {
        ...
        //This formula matches pretty closely to what was originally in render
        //Multiplying pi by time is enough for the angle
        var angle = Math.PI * time;
        var sin = Math.sin(angle);
        sin = Math.pow(sin, 3);
        //MovingSphere's center shouldn't be directly modified.
        //Instead, it's better to create a Vector that has center's properties, then modify that.
        var newCenter = new Vector(this.center.x, this.center.y, this.center.z);
        newCenter.x = newCenter.x + sin * 2;
        return newCenter;
...
class RayTracer {
    ...
    render(scene, encoder: Whammy.Video, length: number, fps: number,
        ...
        //sphere.center.z = sphereCenter.z + Math.cos(0)/2; NEVER USED
        // End >>>>: THIS WILL GO AWAY in your solution, when you add motion blur

        var renderRow = () => {
            for (var x = 0; x < screenWidth; x++) {
                //var color = this.traceRay({ start: scene.camera.pos, dir: getPoint(x, y, scene.camera) }, scene, 0, 0);
                //var c = Color.toDrawingColor(color);
                //Start off with a "brand new" Color
                var c = new Color(0, 0, 0);
                //Using length, frame, and fps, find the current frame
                var currentFrame = ((length * fps) - frame) / fps;
                //Get the next frame, which should be just 0.1 from current (given fps = 10)
                var nextFrame = currentFrame + (1 / fps);
                //Find the current time between currentFrame and nextFrame using a standard Random formula
                //(for refresher, see http://stackoverflow.com/questions/1527803/generating-random-numbers-in-javascript-in-a-specific-range)
                var currentTime = (Math.random() * (nextFrame - currentFrame)) + currentFrame;
                //In the event of anti-aliasing but no motion blur, currentTime can be hard-set to 0.
                //var currentTime = 0;
                
                //Use two for-loops to examine the grid*grid
                for (var p = 0; p < grid; p++) {
                    for (var q = 0; q < grid; q++) {
                        //newX and newY are based of statements in the book's algorithm, which will go into the traceRay.
                        //Math.floor is needed to keep the lined from becoming jagged.
                        var newX = Math.floor(x + (p + Math.random())/grid);
                        var newY = Math.floor(y + (q + Math.random())/grid);
                        //Find the color, using traceRay as an equivalent to the book's ray-color.
                        var color = this.traceRay({start: scene.camera.pos,
                            dir: getPoint(newX, newY, scene.camera)}, scene, 0, currentTime);
                        //Add color to c, using toDrawingColor like the code provided.
                        c = Color.plus(c, Color.toDrawingColor(color));
                    }
                }
                //c needs to be divided by 1/(n^2), with n = grid
...
function defaultScene(): Scene {
    return {
        things: [new Plane(new Vector(0.0, 1.0, 0.0), 0.0, Surfaces.checkerboard),
                 ...
                 new MovingSphere(new Vector(-1.0, 0.5, 1.5), 0.5, Surfaces.checkerboard)],
                 //new Sphere(new Vector(-1.0, 0.5, 1.5), 0.5, Surfaces.shiny)],
        // lights: [{ pos: new Vector(-2.0, 2.5, 0.0), color: new Color(0.49, 0.07, 0.07) },
        //          { pos: new Vector(1.5, 2.5, 1.5), color: new Color(0.07, 0.07, 0.49) },
        //          { pos: new Vector(1.5, 2.5, -1.5), color: new Color(0.07, 0.49, 0.071) },
        //          { pos: new Vector(0.0, 3.5, 0.0), color: new Color(0.21, 0.21, 0.35) }],
        lights: [{ pos: new Vector(-2.0, 2.5, 2.0), color: new Color(0.07, 0.07, 0.7) },
                 { pos: new Vector(1.5, 0.5, 1.5), color: new Color(0.07, 0.7, 0.07) },
                 { pos: new Vector(1.5, 2.5, 0.0), color: new Color(0.7, 0.07, 0.07) },
                 { pos: new Vector(1.0, 3.5, 0.0), color: new Color(0.65, 0.45, 0.45) }],
        // camera: new Camera(new Vector(3.0, 2.0, 4.0), new Vector(-1.0, 0.5, 0.0), 4, 3, 2)
        camera: new Camera(new Vector(3.5, 3.0, 2.0), new Vector(-1.0, 0.5, 0.0), 4, 3, 1)
    };
}
...
function exec() {
    ...
    // start the raytracer
    rayTracer.render(defaultScene(), encoder, length, fps, ctx, 640, 480, 640, 480, 2);
}
